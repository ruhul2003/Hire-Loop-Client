...
export default function PostJobForm({ company }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const targetCompanyId = company?._id || company?.id || company?.data?._id || company?.data?.id;

        if (!targetCompanyId) {
            toast.error("Company information missing. Cannot post job.");
            return;
        }
        ...
        const payload = {
            ...
            companyName: company?.name || company?.data?.name || "Unknown Company",
            companyLogo: company?.logo || company?.data?.logo || "",
            status: "active",
            ...
            <div className="max-w-3xl mx-auto bg-[#121214] border border-zinc-900 rounded-xl p-8 shadow-2xl">
                ...
                <div className="border-b border-zinc-800 pb-6 mb-8">
                    ...
                    <div className="mt-4 inline-flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-400">
                        ...
                        Posting as: <span className="font-semibold text-zinc-300">{company?.name || company?.data?.name || "No Company Selected"}</span>
                    ...
                    <Fieldset className="space-y-6 w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <TextField name="jobTitle" isInvalid={!!errors.jobTitle} className="flex flex-col gap-1 w-full">
                                ...
                                <Input aria-label="Job Title" placeholder="e.g. Senior Frontend Engineer" className={textInputClass} />
